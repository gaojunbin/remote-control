...
struct SessionGroupingTests {
    ...
    @Test("Close is offered on a row the device drives, and on no other")
    func closeIsOfferedOnOneKindOfRow() {
        #expect(SessionListLayout.offersClose(session("driven", control: .remote)))
        // The terminal owns these; the row leaves Active when the CLI exits.
        for control in [SessionControl.terminal, .shared, .none] {
            #expect(!SessionListLayout.offersClose(session("held", control: control)))
        }
        // Nothing in the Archive offers anything: writing to it brings it back.
        #expect(!SessionListLayout.offersClose(session("filed", control: .remote, archived: true)))
        #expect(!SessionListLayout.offersClose(session("filed", control: .terminal, archived: true)))
    }
